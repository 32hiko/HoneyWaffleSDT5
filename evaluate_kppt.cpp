		...
		if (dirty >= PIECE_NO_KING)
		{
			...
			if (pos.side_to_move() == BLACK) {
				if (pos.piece_on(SQ_28) == B_ROOK) {
					diff.p[2][0] += -100 * FV_SCALE;
				}
				else if (pos.piece_on(SQ_27) == B_ROOK) {
					diff.p[2][0] += -100 * FV_SCALE;
				}
				else if (pos.piece_on(SQ_26) == B_ROOK) {
					diff.p[2][0] += -100 * FV_SCALE;
				}
				else if (pos.piece_on(SQ_25) == B_ROOK) {
					diff.p[2][0] += -100 * FV_SCALE;
				}
				else if (pos.piece_on(SQ_24) == B_ROOK) {
					diff.p[2][0] += -100 * FV_SCALE;
				}
				else if (pos.piece_on(SQ_23) == B_ROOK) {
					diff.p[2][0] += -100 * FV_SCALE;
				}
				else if (pos.piece_on(SQ_22) == B_ROOK) {
					diff.p[2][0] += -100 * FV_SCALE;
				}
				else if (pos.piece_on(SQ_21) == B_ROOK) {
					diff.p[2][0] += -100 * FV_SCALE;
				}
				else if (pos.piece_on(SQ_29) == B_ROOK) {
					diff.p[2][0] += -100 * FV_SCALE;
				}
			} else {
				...
				else if (pos.piece_on(SQ_83) == W_ROOK) {
					diff.p[2][0] -= -100 * FV_SCALE;
				}
				else if (pos.piece_on(SQ_84) == W_ROOK) {
					diff.p[2][0] -= -100 * FV_SCALE;
				}
				else if (pos.piece_on(SQ_85) == W_ROOK) {
					diff.p[2][0] -= -100 * FV_SCALE;
				}
				else if (pos.piece_on(SQ_86) == W_ROOK) {
					diff.p[2][0] -= -100 * FV_SCALE;
				}
				else if (pos.piece_on(SQ_87) == W_ROOK) {
					diff.p[2][0] -= -100 * FV_SCALE;
				}
				else if (pos.piece_on(SQ_88) == W_ROOK) {
					diff.p[2][0] -= -100 * FV_SCALE;
				}
				else if (pos.piece_on(SQ_89) == W_ROOK) {
					diff.p[2][0] -= -100 * FV_SCALE;
				}
				else if (pos.piece_on(SQ_81) == W_ROOK) {
					diff.p[2][0] -= -100 * FV_SCALE;
				}
			}
